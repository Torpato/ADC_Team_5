"""
Run with:   python3 simulations/catch_game_FINAL_VERSION.py
"""
...
CATCH_POSE_R = {1: dict(CATCH_POSE),
                2: dict(CATCH_POSE)}
...
def snap_to_hand(r):
    """Poe a bola exatamente no estado de equilibrio da soldadura: posicao,
    ORIENTACAO e velocidade.

    A orientacao e essencial. A soldadura tambem prende a rotacao, e a bola
    chega do voo com uma orientacao qualquer. Sem este ajuste, o solver roda
    a bola de golpe para a orientacao alvo e -- como o ponto de fixacao esta
    12 cm fora do centro da bola -- essa rotacao atira-a para longe (medi
    13 m/s no passo seguinte ao apanho). E depois a soldadura puxa-a de volta.
    Era esse o salto que se via."""
    R = d.xmat[HAND[r]].reshape(3, 3)
    d.qpos[BALL_Q:BALL_Q + 3] = d.xpos[HAND[r]] - R @ m.eq_data[GRIP[r], 3:6]

    q_hand = np.zeros(4)
    mujoco.mju_mat2Quat(q_hand, d.xmat[HAND[r]])
    q_ball = np.zeros(4)
    mujoco.mju_mulQuat(q_ball, q_hand, m.eq_data[GRIP[r], 6:10].copy())
    d.qpos[BALL_Q + 3:BALL_Q + 7] = q_ball
