"""
Run with:   python3 catch_game_V2.py
"""
...
RECOIL_RISE, RECOIL_DECAY = 0.10, 0.35   # sobe em 0.1 s, dissipa em ~0.35 s
...
def add_recoil(pose, dt):
    """Soma o recuo de impacto a pose, com subida rapida e dissipacao suave."""
    if dt < 0:
        return pose
    if dt < RECOIL_RISE:
        a = dt / RECOIL_RISE
...
#          3 bola volta (r1 levanta o braco) | 4 pausa final
reset()
state = 0
...
with mujoco.viewer.launch_passive(m, d) as viewer:
    while viewer.is_running():
        ...
        if d.time < prev_time or d.time > T_TIMEOUT or \
           (state == 4 and d.time - t_catch1 > T_PAUSA):
            reset()
            state = 0
        prev_time = d.time
        ...
        if state <= 2:
            ...
            apply_pose(ctrl, 1, pose_at(d.time, THROW))
        elif state == 3:
            # a bola vem a caminho: sobe o braco de descanso para apanho
            apply_pose(ctrl, 1, mix(REST_POSE, CATCH_POSE, (d.time - t_fly2) / T_RAISE))
        else:
            # apanhou: segura a pose e absorve o impacto
            apply_pose(ctrl, 1, add_recoil(CATCH_POSE, d.time - t_catch1))
        ...
        if state == 0:
            ...
            apply_pose(ctrl, 2, mix(REST_POSE, CATCH_POSE, (d.time - t_fly1) / T_RAISE))
        else:
        ...
        elif state == 2 and d.time - t_catch2 >= T_RELEASE:
            d.eq_active[GRIP[2]] = 0
            m.geom_rgba[BALL_GEOM] = GREEN
            t_fly2 = d.time
            state = 3
